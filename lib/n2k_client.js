...
const socketcan = require("@tamago89/socketcan");
...
class N2K_Client extends EventEmitter {
    ...
    send(msg) {
        if (!(this.candevice && this.options.enableSending)) return;
        if (!(this.candevice.cansend || GLOBAL_PGNS.includes(msg.pgn))) return;
